...
@st.cache_data
def obter_materias_disponiveis():
    try:
        ...
        st.error(f"Erro ao obter matérias disponíveis: {e}")
...
def main():
    ...
    materias_disponiveis = obter_materias_disponiveis()
    
    #assuntos_disponiveis = obter_assuntos_disponiveis(materia_selecionada)
    ...
    with st.expander("Opções de Filtro"):
        ...
        materia_selecionada = st.selectbox("Selecione a matéria:", materias_disponiveis)
        # Adicionar um seletor para o filtro de assunto
        assunto_filtrado = st.multiselect("Selecione o assunto (ou deixe vazio para todos)", obter_assuntos_disponiveis(materia_selecionada), help='Se nenhum assunto for selecionado, serão exibidas questões de todos os assuntos.')
        
        # Adicionar um seletor para o filtro de assunto
        topico_filtrado = st.multiselect("Selecione o tópico (ou deixe vazio para todos)", obter_topicos_disponiveis(materia_selecionada), help='Se nenhum assunto for selecionado, serão exibidas questões de todos os tópicos.')

        # Adicione um aviso informativo sobre a seleção de todos os assuntos
    
        if not assunto_filtrado:
            assunto_filtrado = obter_assuntos_disponiveis(materia_selecionada)
        if not topico_filtrado:
            topico_filtrado = obter_topicos_disponiveis(materia_selecionada)
        # Obter a quantidade máxima de questões com base no assunto filtrado

        quantidade_maxima = obter_quantidade_maxima_questoes(materia_selecionada,assunto_filtrado,topico_filtrado)
    ...
    questoes = obter_todas_questoes(materia_selecionada, assunto_filtrado, topico_filtrado, quantidade_questoes)
